...
|:------------:|:--------------|:--------------|:-------------------------------:|:--------------------------:|
| `w = 1`      | $=\frac{1}{2m} [0^2 + 0^2 + 0^2]\\=\frac{0}{2m}\\=0$                               | For each value of `i`, `x` and `y` is `1`, `2` & `3`.<br/>But `w` remain constant i.e. `1`.<br/>So, for $(wx^{(i)} - y^{(i)})^2$, each value will be $(1(x) - y)^2$, and result will be `0`.            | <img src="./images/regression-line-w-1-cost-function.jpg" alt="regression-line-1">            | <img src="./images/cost-function-w-1.jpg" alt="cost-function-1"> |
| `w = 0.5`    | $=\frac{1}{2m} [0.25 + 1 + 2.25]\\=\frac{0}{2m} [3.5]\\=\frac{3.5}{6}\\={0.58}$    |  For each value of `i`, `x` and `y` is `1`, `2` & `3`.<br/>But `w` remain constant i.e. `1`.<br/>So, for $(wx^{(i)} - y^{(i)})^2$, each value will be <br/>`x=1`, $\\(0.5(1) - 1)^2 = (0.5 - 1)^2 = 0.25\\$ `x=2`, $\\(0.5(2) - 2)^2 = (1 - 2)^2 = 1\\$ and `x=3`, $\\(0.5(3) - 3)^2 = (1.5 - 3)^2 = 2.25\\$ and result will be `0.58` approx.            | <img src="./images/regression-line-w-2-cost-function.jpg" alt="regression-line-2">            | <img src="./images/cost-function-w-2.jpg" alt="cost-function-2"> |
| `w = 0`      | $=\frac{1}{2m} [(1)^2 + (2)^2 + (3)^2]\\=\frac{0}{2m} [14]\\=\frac{14}{6}\\={2.3}$ | For each value of `i`, `x` and `y` is `1`, `2` & `3`.<br/>But `w` remain constant i.e. `1`.<br/>So, for $(wx^{(i)} - y^{(i)})^2$, each value will be <br/>`x=1`, $\\(0(1) - 1)^2 = (1)^2\\$ `x=2`, $\\(0(2) - 2)^2=(2)^2\\$ and `x=3`, $\\(0(3) - 3)^2=(3)^2$ and result will be `2.3` approx.<br/> And because `w=0`, so any value of `x` multiplied by `w` resulted `0`. <br/> Hence, our *regression lin* will be made on `x-axis`.            | <img src="./images/regression-line-w-3-cost-function.jpg" alt="regression-line-3">            | <img src="./images/cost-function-w-3.jpg" alt="cost-function-3"> |
